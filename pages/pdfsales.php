 ...
 class PDF extends FPDF
 {
 	...
 	function Header()
 	{
       ...
       // $this ->Image('img/tb.jpg',10,10,10);
        ...
        $this ->Cell(176,10,'Cash Sales Report',0,0,'C');
        $this ->Ln(20);
        
    }
 		$cone = mysqli_connect("localhost","root","","abel");
 	   $result = mysqli_query($cone,"SELECT sum(total) FROM sales") or die(mysqli_error());
        while ($rows = mysqli_fetch_array($result)) {
        $this ->Cell(176,10,'Total Sales Tsh:',0,0,'C');
        $this ->Ln(5);
